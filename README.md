...
class module(ModuleType):
    ...
    def __dir__(self):
        ...
        return result
 ```
 这还只是导入模块的部分，但是我连为什么这样做不清楚，其他函数看了一下发现都是一些底层原理的实现，很多概念我都没有，这样看效果太低，挫折感太强了，我一直都是这样干的，这样肯定不会学会，我又陷入<a href='#'>先把所有东西都学好，再回来搞</a>这样的怪圈中了，再这样下去我估计我要看到计算是怎么一回事情了，要搞玄学了！<br\>
 
 <em>不能散开精力干这种效率很低的事情，首要目标是先建立网站至于底层实现，服务器原理什么的，打包结构，暂且不管以后再说</em>
...
不给力啊，下面要怎么做还是不知道，我看到的博客在不包括前端代码的情况下包含，`用户注册部分，文章上传部分，控制台部
分,标签部分,文章搜索部分,时间相关的部分`等等，不知道怎么下手啊，结合FLASK web开发这本书吧，不行去网上down一个down一个例子下来抄一下，虽然目标是做一个博客但必须把目标细分,不然又要思密达了，这个留着去图书馆再写吧


#####计算机科学概论学习

------
今天主要看了数据存储的前三小节，目前到二进制(P29)


###8月16日对于建立网站的理解

------
网页数据都在本地，人怎么通过在浏览器输入一个url就能够把他想要的资源给传递过去的.<br/>

一个html页面在本地构成，然后被存储在硬盘里面
#####这里就涉及到了html
* html页面怎么写
* html页面怎么传递过去
* 网页里要用到一些动态的数据怎么办

这时候人在浏览器输人url，然后浏览器发送了一次http请求

#####这里面就涉及到了http
* 怎么传递http请求
* http请求有相应的格式怎么满足

server端回复响应，然后将被请求的资源传递过去。
#####server端的建立
* server端时如何运行的

请求人在页面上操作会产生一些数据，这些数据需要保存起来
#####这里面涉及到了数据库
* 数据库怎么建立
* 怎么和数据库建立连接
* 怎么操作数据库的读写，检索

再次登陆的时候，是可以设置cookie纪录账号信息的，还有验证问题
#####这里涉及到了cookie,验证完全性
* cookie 的配置
* 如何验证,如何确保安全性
* 邮箱，手机，验证码，都是如何实现的
...
------
以上这些都是需要人去写的,每一个细节都是要人去具体实现的,这还只是我简单的想了一下.如果数据量大了，肯定更加复杂，更多细节需要考虑<br/>
框架肯定是为人服务的，肯定是为了减少工作量的，而且一个成熟的框架肯定个方面都比较完善了.刚开始学习一个框架的时候肯能并不理解一些细节，那是因为刚开始学习的只是很简单的东西,用不到那些复杂的功能,但其实存在都是有意义的，所以不明白一些地方的时候`尽可能的多想想为什么存在这个东西，意义是什么`.<br/>
